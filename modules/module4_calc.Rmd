...
2) Compute the number of NAs in each column of the *air* dataset using `sapply()` and the `is.na()` function.

3) Use `table()` to figure out the number of flights for each year.
...
4) Discretize distance into some bins and create a Dist_binned variable. Count the number of flights in each bin.

5) Create a boxplot of delay by binned distance.

6) Consider the *numFlights* data frame that I created. Sort this by the number of flights and look to see which are the most frequent destinations.
...
8) For each combination of UniqueCarrier, Month, and DayOfWeek, find the 95th percentile of delay. 

9) This requires a bit of setup. Run the following code:
h <- air$DepTime %/% 100
m <- air$DepTime %% 100
h <- as.character(h)
...
Now suppose this is the format the departure time field came in.
