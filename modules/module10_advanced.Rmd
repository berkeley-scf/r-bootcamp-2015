...
```{r}
rboot2015 <- list(month = 'August', year = 2015, 
  instructor = 'Paciorek', attendance = 100)
class(rboot2015) <- "workshop"

rboot2015
is(rboot2015, "workshop")
rboot2015$instructor 
...
for(curDest in Dests) {
            ...
            if(is(tmp, "try-error")) mod[[curDest]] <- NA else mod[[curDest]] <- curMod            
}
...
If you have access to multiple machines within a networked environment, such as the compute servers in the Statistics Department or the campus-wide Savio cluster or machines on Amazon's EC2 service, there are a couple straightforward ways to parallelize EP jobs across machines.
