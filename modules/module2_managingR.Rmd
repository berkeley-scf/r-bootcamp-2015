...
# Linux/Mac specific
setwd('~/Desktop/r-bootcamp-2015') # change the working directory
setwd('/Users/paciorek/Desktop/tmp') # absolute path
getwd()
setwd('../r-bootcamp-2015') # relative path

# platform-agnostic
setwd(file.path('~', 'Desktop', 'r-bootcamp-2015')) # change the working directory
setwd(file.path('/', 'Users', 'paciorek', 'Desktop', 'tmp')) # absolute path
getwd()
setwd(file.path('..', 'r-bootcamp-2015')) # relative path
```
...
We'll go through a short example of making changes to the r-bootcamp-2015 repository. In this case you don't have permission to make changes so you'll just have to follow along as I do it. However, you could start your own repository and then you'd be able to do similar things.
...
2) Put the data/cpds.csv file in some other directory on your computer (or the VM if using the VM), such as *Downloads*. Use `setwd()` to set your working directory to be that directory. Read the file in using `read.csv()`.  Now use `setwd()` to point to a different directory such as *Desktop*. Write the data frame out to a file but without any row names and without quotes on the character strings.

3) Make a plot with the airline data. Save it as a PDF in *Desktop*. Now see what happens if you set the *wdith* and *height* arguments to be very small and see how it affects the resulting PDF. Do the same but setting width and height to be very large.

4) Figure out where (what directory) the *graphics* package is stored on your machine. Is is the same as where the *fields* package is stored?
